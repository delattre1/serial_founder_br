...
/commit justfile
/commit src/main.py tests/test_main.py README.md
```
...
1. If arguments are provided via $ARGUMENTS, adds those specific files with `git add <files>`
...
1. Run `git add $ARGUMENTS` to stage the specified files
2. Run `git diff --staged` to see what will be committed
3. Analyze the changes and create appropriate commit message(s)
